...
- **Weekly reset**: the reward week ends at EVENING_START_HOUR (18:00) on the family's reset day — Saturday by default because that's when parents hand out the reward and the evening list flips over in the same moment. The day is configurable in Parent mode (Inställningar → Veckan slutar), stored as `state.resetSchedule` = `[{ effectiveAt, day }]`: a change appends an entry effective at the end of the week in progress, so no week is ever cut short and no reward moves or is lost; only the transition week can be shorter/longer. `getRewardWeekStart` / `getNextRewardWeekEnd` / `getPreviousRewardWeekStart` all take an optional `(now, schedule)` and use calendar days, not 7×24h, so a DST change can't move a boundary off 18:00. Verified: the default schedule matches the old Saturday algorithm on 5,000 random moments; every weekday change; reward attribution across a transition; the exact cutoff second; DST. The kid card header shows `weekDaysLeftText()` ("3 dagar kvar", "slutar i kväll").
