...
- [ ] Seasonal and holiday visual themes (winter + Santa, summer, World Cup, etc.) — build on top of the token system from section 4 so a theme is a swap, not a rewrite. Ties into a mascot character (see §13): whether a reskin needs a new App Store build depends entirely on where its assets live, not on Lottie-vs-custom-SVG. If the mascot's data (which of N base characters, which seasonal costume is active) is fetched at runtime — a small manifest on GitHub Pages, or a Supabase row — a reskin is a content edit only: no new app version, no Apple review, nothing for the family to update, same as any other web change. If assets are instead baked into the native bundle (inline SVG in `app.js`, or Lottie JSON shipped as static files under `www/`), every reskin needs `npm run cap:sync` → an Xcode archive → App Store review → the family updating the app before they see it. Only the renderer logic needs a native build the first time either way — build the mascot system once, then drive "which asset(s) are active today" from a remotely-fetched manifest to keep reskins free after that. Doesn't apply to the web app at all — it's never bundled, so any `app.js` change there is already live on next load.
...
- [ ] **Per-session moment** (i.e. §3's "extra feedback on the first tick of the day", generalized to the *last* tick too): the current "Allt klart för idag! 🎉" text is the whole payoff for finishing every task — make this bigger and more noticeable, the actual "win" moment of the whole session. Fold in a fix for the current styling while redesigning this, rather than patching it separately: the coral outline border around the reward card (`.reward-card.celebrating`) reads as off-brand — nothing else in the app has an outline like it — and the celebration text's font doesn't match the task list around it
